...
def find_latest_file(epoch, path):
    files = glob.glob(os.path.join(path, "epoch={:06}_*.npz".format(epoch)))
    if not files:
        return None
    latest_number = max(int(file.split('_')[-1].split('.')[0]) for file in files)
    return latest_number

def file_path(epoch, path):
    latest_number = find_latest_file(epoch, path)
    next_number = latest_number + 1 if latest_number is not None else 1
    
    new_file_name = "epoch={:06}_{}.npz".format(epoch, next_number)
...
class DDPM(pl.LightningModule):
    ...
    def p_losses(self, x_start, t, noise=None):
        ...
        loss = self.get_loss(eps_pred, target, mean=False).mean(dim=[1, 2, 3])
...
class LatentDiffusion(DDPM):
    ...
    def get_learned_conditioning(self, c):
        if self.cond_stage_forward is None:
            ...
            assert hasattr(self.cond_stage_model, self.cond_stage_forward)
            if self.lambda_quant is not None:
                c, quantized_loss = getattr(self.cond_stage_model, self.cond_stage_forward)(c)
                return c, quantized_loss
            
            elif self.gaussian is not None:
                c, kl_loss = getattr(self.cond_stage_model, self.cond_stage_forward)(c)
                return c, kl_loss
            else:
                c = getattr(self.cond_stage_model, self.cond_stage_forward)(c)
                return c
    ...
    def forward(self, x, c, *args, **kwargs):
        ...
        if self.model.conditioning_key is not None:
            ...
            if self.cond_stage_trainable:
                if self.lambda_quant is not None:
                    c, quantized_loss = self.get_learned_conditioning(c)
                elif self.gaussian is not None:
                    c, kl_loss = self.get_learned_conditioning(c)
                else:
                    c = self.get_learned_conditioning(c)
            if self.shorten_cond_schedule:  # TODO: drop this option
                tc = self.cond_ids[t].to(self.device)
                c = self.q_sample(x_start=c, t=tc, noise=torch.randn_like(c.float()))
        return self.p_losses(x, c, t, quantized_loss=quantized_loss, kl_loss=kl_loss, *args, **kwargs)
    ...
    def apply_model(self, x_noisy, t, cond, return_ids=False, sampled_concept= None, sampled_index= None):
        ...
        if hasattr(self, "split_input_params"):
            ...
            x_recon = self.model(x_noisy, t, sampled_concept = sampled_concept, sampled_index = sampled_index, **cond)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if self.lambda_quant is not None:
            cond, _ = self.cond_stage_model(x)
    ...
    def validation_epoch_end(self, validation_step_outputs):
        cond_cat = torch.cat(validation_step_outputs, dim=0)
        cond_dir = os.path.join(self.logdir, "dis_repre","epoch={:06}.npz".format(
                self.current_epoch))
        os.makedirs(os.path.join(self.logdir, "dis_repre"), exist_ok=True)
        if self.trainer.val_check_interval < 1 and self.trainer.val_check_interval > 0:
            cond_dir = file_path(self.current_epoch, os.path.join(self.logdir, "dis_repre"))
    ...
    def p_losses(self, x_start, cond, t, quantized_loss=None, kl_loss=None, noise=None):
        noise = default(noise, lambda: torch.randn_like(x_start))
        x_noisy = self.q_sample(x_start=x_start, t=t, noise=noise)
        ...
        loss_simple = self.get_loss(eps_pred, target, mean=False).mean([1, 2, 3])
        loss_dict.update({f'{prefix}/loss_simple': loss_simple.mean()})
        if self.lambda_quant is not None:
            loss_quant = quantized_loss.mean()
            loss_dict.update({f'{prefix}/loss_quant': loss_quant})
        if self.gaussian is not None:
            loss_kl = kl_loss
            loss_dict.update({f'{prefix}/loss_kl': loss_kl})
        logvar_t = self.logvar[t.cpu()].to(self.device)
        if self.lambda_quant is not None:
            loss = loss_simple / torch.exp(logvar_t) + logvar_t + self.lambda_quant * loss_quant
        elif self.gaussian is not None:
            loss = loss_simple / torch.exp(logvar_t) + logvar_t + self.gaussian * loss_kl
        else:
        ...
        loss_vlb = self.get_loss(eps_pred, target, mean=False).mean(dim=(1, 2, 3))
    ...
    @torch.no_grad()
    def log_images(self, batch, N=8, n_row=8, sample=True, ddim_steps=200, ddim_eta=1., return_keys=None,
...
class DiffusionWrapper(pl.LightningModule):
    ...
    def forward(self, x, t, sampled_concept = None, sampled_index = None, c_concat: list = None, c_crossattn: list = None):
        if self.conditioning_key is None:
            ...
            out = self.diffusion_model(x, t, context=cc, **model_kwargs)
        else:
            raise NotImplementedError()
