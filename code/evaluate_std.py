...
def eval_func(eval_dataset, data_tensor, metric_folder, it, gi, preflix=""):
    ...
    if beta_VAE_score:
        ...
        result_dict = compute_beta_vae_sklearn(eval_dataset,_representation,random_state=np.random.RandomState(gi.seed),artifact_dir=None)
    ...
    if factor_VAE_score:
        ...
        result_dict = compute_factor_vae(eval_dataset,_representation,random_state=np.random.RandomState(gi.seed),artifact_dir=None)
    ...
    if dci_score:
        ...
        result_dict = compute_dci(eval_dataset,_representation,random_state=np.random.RandomState(gi.seed),artifact_dir=None)
    ...
    if MIG_score:
        ...
        result_dict = compute_mig(eval_dataset,_representation,random_state=np.random.RandomState(gi.seed),artifact_dir=None)
    ...
    if Modularity_score:
        with gin.unlock_config():
            ...
            gin.bind_parameter("discretizer.discretizer_fn",_histogram_discretize)
            gin.bind_parameter("discretizer.num_bins",20)
        result_dict = compute_modularity_explicitness(eval_dataset,_representation,random_state=np.random.RandomState(gi.seed),artifact_dir=None)
    ...
    if SAP_score:
        ...
        result_dict = compute_sap(eval_dataset,_representation,random_state=np.random.RandomState(gi.seed),artifact_dir=None)
    ...
    if infoMEC_score:
        ...
        result_dict = compute_infomec(eval_dataset, _representation, np.random.RandomState(gi.seed),discrete_latents=False,test_size=10000)
...
if __name__ == "__main__":
    ...
    def evaluate_process(label_dataset, args):
        ...
        result_dict = eval_func(label_dataset, latents, os.path.join(args.exp_dir, "results"), "results-{:06d}".format(args.epoch), gi = args)
        
        save_dict = OrderedDict({'index': args.seed})
        save_dict.update(result_dict)
        #os.makedirs(os.path.join(args.exp_dir, str(args.seed)), exist_ok=True)
        append_dict_to_csv(save_dict, os.path.join(args.exp_dir, "std_metrics.csv"))
    ...
    def evaluate_tad(dataset, args):
        ...
        plt.savefig(os.path.join(args.exp_dir, "results", 'tad_image-{:06d}.jpg'.format(args.seed)))
    ...
    # plot_metric(os.path.join(args.exp_dir,str(args.seed),"metrics.csv"))
    # plot_loss(args.exp_dir)