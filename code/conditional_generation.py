...
def make_convolutional_sample(model, batch_size, vanilla=False, custom_steps=None, eta=1.0, opt=None):
    ...
    with model.ema_scope("Plotting"):
        ...
        if vanilla:
            ...
            sample, intermediates, samples = convsample_ddim(model,  steps=custom_steps, shape=shape,
                                                    eta=eta, opt=opt)
...
def run(model, logdir, batch_size=50, vanilla=False, custom_steps=None, eta=None, n_samples=50000, nplog=None, opt=None):
    ...
    for _ in trange(n_samples // batch_size, desc="Sampling Batches (unconditional)"):
        logs, x_cat = make_convolutional_sample(model, batch_size=batch_size,
                                            vanilla=vanilla, custom_steps=custom_steps,
                                            eta=eta, opt=opt)
        x_cat = custom_to_pil(x_cat.squeeze(0))
        x_cat.save(os.path.join(logdir, f"inter_{n_saved:06}.png"))
